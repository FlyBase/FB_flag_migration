...
require Exporter;
our @ISA = qw(Exporter);
our @EXPORT = qw(make_abc_json_metadata get_yyyymmdd_date_stamp pub_has_curated_data get_relevant_curator_from_candidate_list_using_pub_and_timestamp check_and_validate_nocur);
...
=cut
	...
	if (exists $data->{$pub_id} && exists $data->{$pub_id}->{$timestamp}) {
		...
		if ($relevant_curator) {
			...
			if ($relevant_curator eq 'P. Leyland') {
				$relevant_curator = "FB_curator";
